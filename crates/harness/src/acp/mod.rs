...
//! KEPT ONLY for agents built ground-up on ACP: Grok ([`AcpHarness::grok`],
//! `grok agent stdio`) and Hermes ([`AcpHarness::hermes`], `hermes acp`). The
//! legacy [`AcpHarness::pi`] constructor remains for compatibility, but the
//! registry uses Pi's native JSONL RPC driver; opencode moved to its native
//! HTTP/SSE driver ([`crate::OpencodeHarness`]). Claude, Codex and Cursor moved to native drivers
